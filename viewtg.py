...
logo=(f'''{B}{E}========================================{E}
  ...
  {E}𝐓ʜɪ𝐬 𝐒ᴄʀɪᴘᴛ 𝐈𝐬 𝐎ᴡɴᴇᴅ 𝐁ʏ TLC ARMY{B}
       
         {G}[+] 𝐃ᴇᴠᴇʟᴏᴘᴇʀ -> {B}𝐌𝐫.BRONO                        
   
         {G}[+] 𝐓ᴇʟᴇɢʀᴀᴍ 𝐔sᴇʀɴᴀᴍᴇ -> {B}@TLC_OWNER     
        
    	 {G}[+] 𝐓ᴇʟᴇɢʀᴀᴍ 𝐂ʜᴀɴɴᴇʟ -> {B}@tlc_army   
    	 
{E}==================================================''')
def tui():
    while True:
        print(logo)
        print(f'''{B}[ 𝐃ᴀᴛᴀ ]: {G}{channel.capitalize()}/{post}
{B}[ 𝐋ɪᴠᴇ 𝐕ɪᴇᴡs ]: {G}{real_views} ✅
 
{S}[ 𝐂ᴏɴɴᴇᴄᴛɪᴏɴ 𝐄ʀʀᴏʀs ]: {E}{proxy_errors} 🚫
{S}[ 𝐓ᴏᴋᴇɴ 𝐄ʀʀᴏʀs ]: {E}{token_errors} ❌
 
{G}[ 𝐓ʜʀᴇᴀᴅs ]: {B}{active_count()} ⇝⇝⇝⇝ ''')
        sleep(2);system('cls' if name == 'nt' else 'clear')

system('cls' if name == 'nt' else 'clear')
print(logo)
channel, post = input(f"\n{G}[+] 𝐓ᴇʟᴇɢʀᴀᴍ 𝐕ɪᴇᴡ 𝐏ᴏsᴛ 𝐔ʀʟ -> {S}").replace('https://t.me/', '').split('/')
...
try:
    ...
    print(f'{G}[√]𝐒ᴛᴀʀᴛᴇᴅ | 𝐖ᴀɪᴛ 𝐅ᴇᴡ 𝐒ᴇᴄᴏɴᴅs 𝐓ᴏ 𝐑ᴜɴ 𝐓ʜʀᴇᴀᴅs')
